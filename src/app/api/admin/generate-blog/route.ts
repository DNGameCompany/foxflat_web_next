...
КРИТИЧНО для JSON: в HTML атрибутах у полі content використовуй одинарні лапки (<a href='url'>, <img src='...'>), а НЕ подвійні — подвійні лапки всередині JSON-рядка ламають парсинг.
...
function repairAndParse(raw: string): unknown {
    // step 1: escape real newlines/tabs
    const s = raw
        .replace(/\r\n/g, "\\n")
        .replace(/\n/g, "\\n")
        .replace(/\r/g, "\\r")
    ...
    // step 2: unescaped " inside "content" field — replace with &quot;
    // content is always followed by ","category"
    const fixed = s.replace(
        /("content"\s*:\s*")(.*?)(",\s*"category")/,
        (_, pre, inner, post) => pre + inner.replace(/(?<!\\)"/g, "&quot;") + post,
    );
